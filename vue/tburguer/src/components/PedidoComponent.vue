    ...
    <div>
        <form id="pedido-form" @submit="criarPedido($event)">
            ...
            <div class="inputs" id="form-pedido">
                ...
                <p v-if="!nomeCliente && showValidationErrors" class="error-message">Nome é obrigatório.</p>
            </div>
            <div class="inputs">
                ...
                <select id="ponto-carne" name="ponto-carne" v-model="pontoCarneSelecionado">
                    ...
                    <option v-for="pontoCarne in listaPontoCarne" :key="pontoCarne.id" :value="pontoCarne">{{
                        pontoCarne.descricao }}</option>
                </select>
                <p v-if="!pontoCarneSelecionado && showValidationErrors" class="error-message">Ponto da carne é
                    obrigatório.</p>
            </div>
            <div id="opcionais-titulo" class="inputs">
                ...
                <div class="checkbox-container" v-for="complemento in listaComplementos" :key="complemento.id">
                    <input type="checkbox" :name="complemento.nome" v-model="listaComplementosSelecionados"
                        :value="complemento">
        ...
        <button v-if="exibirBotaoLista" @click="navegarParaListaPedidos" class="navegar-btn">
    ...
    methods: {
        ...
        async criarPedido(e) {
            ...
            try {
                ...
                if (requisicao.ok) {
                    this.mensagem = { tipo: 'sucesso', texto: 'Pedido criado com sucesso!' };
                    this.exibirBotaoLista = true;
                    this.showValidationErrors = false;
                } else {
                    throw new Error('Erro ao criar pedido.');
                }
            } catch (error) {
                this.mensagem = { tipo: 'erro', texto: 'Erro ao criar pedido. Tente novamente.' };
            }
        },
